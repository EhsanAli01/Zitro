...
const HomeBody = () => {
    ...
    const filterHandler = () => {
        ...
        if (isActive) setFilteredProducts(products);
        else setFilteredProducts(filteredProducts);
    }
        ...
        <main className='h-5/6'>
            <section className='h-24 mt-4 flex justify-between items-center'>
                <div className='mx-24 h-8 border border-blue-950 flex items-center rounded-full overflow-hidden font-semibold max-sm:text-sm max-sm:mx-10'>
                    <button onClick={filterHandler} className={`flex items-center px-3 py-2 h-full ${isActive ? '' : 'bg-slate-800 text-white'}`}>Posted</button>
                    <button onClick={filterHandler} className={`flex items-center px-3 py-2 h-full ${isActive ? 'bg-slate-800 text-white' : ''}`}>Active</button>
                </div>
